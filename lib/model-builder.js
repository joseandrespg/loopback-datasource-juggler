      ...
      set: function(value) {
        ...
        if (ModelClass.setter[propertyName]) {
          ...
          if (value === null || value === undefined) {
            ...
            if (DataType === List) {
              this.__data[propertyName] = DataType(value, properties[propertyName], this.__data);
            } else {
              // Assume the type constructor handles Constructor() call
              // If not, we should call new DataType(value).valueOf();
              this.__data[propertyName] = (value instanceof DataType) ? value :
                DataType(value, properties[propertyName]);
            }
          }
        }
      },
...
function DateType(arg, props) {
  ...
  var d = new Date(arg);
  if (isNaN(d.getTime())) {
    return props.isDateString ? 'Invalid Date' : d;
  }
  return props.isDateString ? arg : d;
}
